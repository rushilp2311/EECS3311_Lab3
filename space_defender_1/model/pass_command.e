		...
		do
			...
			if model.cursor - 1 > 0  then
				if (model.location[model.cursor].old_l.row /= model.location[model.cursor].curr.row) and (model.location[model.cursor].old_l.column /= model.location[model.cursor].curr.column) then
						model.output_msg.append ("%N"+"  The Starfighter moves: "+"["+model.row_indexes.item (model.location[model.cursor-1].old_l.row).out+","+model.location[model.cursor-1].old_l.column.out+"]"+" -> "+"["+model.row_indexes.item (model.ship_location.row).out+","+model.ship_location.column.out+"]")
				end
			end
